...
Slack.prototype._request = function (options, callback) {
  ...
  var colors = {'error_color':'#EE5F5B','warn_color':'#F89406'} // Hard coded for now
  ...
  var attachments = this.options.attachments ? JSON.parse( _.template(JSON.stringify(this.options.attachments), options.params, {interpolate: /\{\{(.*?)\}\}/g})) : null
  attachments.forEach(function(attach){
    if(colors.hasOwnProperty(attach.color)){
      attach.color = colors[attach.color]
    }else{
      attach.color = '#7A8288'
    }
  })
  options.body = JSON.stringify({
    ...
    text: this.options.message == null? null : this.options.message ? _.template(this.options.message, options.params, {
      interpolate: /\{\{(.*?)\}\}/g
